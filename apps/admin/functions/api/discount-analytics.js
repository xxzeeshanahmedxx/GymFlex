...
export async function onRequestGet(context) {
  ...
  const [usage, topCoupons, summary] = await Promise.all([
    db.prepare("SELECT d.id, d.code, d.discount_percent, d.is_active, d.expires_at, COUNT(o.id) AS usage_count, COALESCE(SUM(o.discount_amount), 0) AS total_discount FROM discounts d LEFT JOIN orders o ON o.discount_code = d.code GROUP BY d.id ORDER BY usage_count DESC").all(),
